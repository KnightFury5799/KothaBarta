...
async function generatePhotocard(text) {
  const w=1080,h=1080;
  const canvas=document.createElement("canvas");
  canvas.width=w;canvas.height=h;
  const ctx=canvas.getContext("2d");
  const bg=ctx.createLinearGradient(0,0,w,h);
  bg.addColorStop(0,"#08060F");bg.addColorStop(0.5,"#110E22");bg.addColorStop(1,"#1a1035");
  ctx.fillStyle=bg;ctx.fillRect(0,0,w,h);
  const g1=ctx.createRadialGradient(200,200,0,200,200,400);
  g1.addColorStop(0,"#F0A50018");g1.addColorStop(1,"transparent");
  ctx.fillStyle=g1;ctx.fillRect(0,0,w,h);
  const g2=ctx.createRadialGradient(900,900,0,900,900,350);
  g2.addColorStop(0,"#E0608015");g2.addColorStop(1,"transparent");
  ctx.fillStyle=g2;ctx.fillRect(0,0,w,h);
  ctx.fillStyle="#161240CC";
  ctx.beginPath();ctx.roundRect(80,80,w-160,h-160,40);ctx.fill();
  ctx.strokeStyle="#F0A50030";ctx.lineWidth=2;
  ctx.beginPath();ctx.roundRect(80,80,w-160,h-160,40);ctx.stroke();
  ctx.strokeStyle="#F0A500";ctx.lineWidth=4;
  ctx.beginPath();ctx.moveTo(200,185);ctx.lineTo(880,185);ctx.stroke();
  ctx.fillStyle="#F0A500";ctx.font="bold 40px Georgia,serif";ctx.textAlign="center";
  ctx.fillText("kothabarta",w/2,158);
  ctx.fillStyle="#7A7295";ctx.font="26px Georgia,serif";
  ctx.fillText("কথাবার্তা",w/2,235);
  ctx.fillStyle="#F5EFE0";ctx.font="52px Georgia,serif";ctx.textAlign="center";
  const words=text.split(" ");const lines=[];let line="";const maxW=w-240;
  ...
  const lineH=72;const totalH=lines.length*lineH;const startY=(h-totalH)/2+20;
  lines.forEach((l,i)=>ctx.fillText(l,w/2,startY+i*lineH));
  ctx.strokeStyle="#2A2448";ctx.lineWidth=1;
  ctx.beginPath();ctx.moveTo(200,h-220);ctx.lineTo(880,h-220);ctx.stroke();
  ctx.fillStyle="#7A7295";ctx.font="28px sans-serif";ctx.textAlign="center";
  ctx.fillText("anonymous message",w/2,h-165);
  ctx.fillStyle="#3A3560";ctx.font="22px monospace";
  ctx.fillText("kotha-barta-one.vercel.app",w/2,h-115);
...
function MsgCard({ msg, onDelete, onRead }) {
  ...
  const handleShare=async(e)=>{
    ...
    try{
      const dataUrl=await generatePhotocard(msg.text);
      const blob=await(await fetch(dataUrl)).blob();
      const file=new File([blob],"kothabarta-message.png",{type:"image/png"});
      if(navigator.share&&navigator.canShare({files:[file]})){
        await navigator.share({files:[file],title:"Anonymous message on kothabarta"});
      } else {
        const a=document.createElement("a");a.href=dataUrl;a.download="kothabarta-message.png";a.click();
      }
    } catch{}
    setSharing(false);
  };
  ...
  return (
    <div onClick={()=>onRead(msg.id)} style={{ background:msg.read?T.card:"#161240",border:`1px solid ${msg.read?T.border:T.accent+"40"}`,borderRadius:14,padding:"18px 20px",marginBottom:12,cursor:"pointer",transition:"all 0.2s",position:"relative",animation:"fadeIn 0.35s ease" }}>
      ...
      <p style={{ color:T.text,fontSize:15,lineHeight:1.7,marginBottom:14 }}>{msg.text}</p>
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center" }}>
        <span style={{ color:T.muted,fontSize:12 }}>{timeStr}</span>
        <div style={{ display:"flex",gap:8,alignItems:"center" }} onClick={e=>e.stopPropagation()}>
          <button onClick={handleSave} style={{ color:T.muted,fontSize:11,background:"#0D0A1E",border:`1px solid ${T.border}`,borderRadius:8,padding:"5px 10px",cursor:"pointer" }}>💾 Save</button>
          <button onClick={handleShare} disabled={sharing} style={{ color:"#08060F",fontSize:11,background:T.accent,border:"none",borderRadius:8,padding:"5px 10px",cursor:"pointer",fontWeight:600 }}>{sharing?"…":"↗ Share"}</button>
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    if(match){
      ...
      setSendTo(username);
      setView("compose");
      DB.getUser(username).then(u=>{
        if(u) setSendTarget({username:u.username,displayName:u.display_name});
      });
      return;
    }
    // Restore session
